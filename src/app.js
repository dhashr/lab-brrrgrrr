...
function renderAll(name,state) {
  renderPatty();
  renderCheese();
  renderTomatoes();
  renderOnions();
  renderLettuce();
  renderButtons(name,state);
  renderIngredientsBoard(name,state);
  renderPrice();
}
...
function renderButtons(btnName,state){ 
  document.querySelector(`.${btnName}`).classList.toggle("active");

}

//Challenge 2 - Render only the items selected in the ingredients board based on the state
function renderIngredientsBoard(name,rtn){

  let ingredients = document.querySelectorAll(".items");
  ingredients.forEach((ingredient) =>{
    let ingredientitems = ingredient.innerText;
    if(state[ingredientitems] == true){
